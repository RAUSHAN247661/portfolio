...
const menu = navItems.map((item) => (
  <li key={item.id} onMouseDown={(e) => e.preventDefault()}>
    <Link
      ...
      className={`hover:text-picto-primary px-5 py-3 mx-1 hover:rounded-full`}
    >
      {item.name}
    </Link>
  </li>
    ...
    >
      <div className="navbar flex justify-between mx-auto content">
        ...
        <div className="lg:flex items-center">
          ...
          <p className="">
            <Link
              className="btn btn-sm xs:btn-md sm:btn-lg rounded-full bg-blue-500 text-white font-medium hover:bg-blue-600"
              href="#contact"
              to={`contact`}
              smooth={true}
              duration={900}
